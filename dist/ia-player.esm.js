...
`;for(let p of r){let v=i.get(p.slug),y=v?`
    ...
    dct:date ${it(p.year)} ;`:"",b=p.tracks.map((f,L)=>`<#t${String(L+1).padStart(2,"0")}>`).join(", "),h=`@prefix dct: <http://purl.org/dc/terms/> .
@prefix mo: <http://purl.org/ontology/mo/> .
@prefix dcat: <http://www.w3.org/ns/dcat#> .
@prefix foaf: <http://xmlns.com/foaf/0.1/> .
...
<#it>
    ...
    mo:track ${b} .
`;p.tracks.forEach((f,L)=>{let $=`t${String(L+1).padStart(2,"0")}`,D=f.trackNo!=null?f.trackNo:L+1,z=f.durationSec!=null?`
...
`));if(!ue.ok)return Be(ue);let he=[M(l.releasesCatalog,de("dataset"),S.releaseNode,l.releasesDoc)],_e=await Ae(e,l.releasesDoc.value,[],he);if(!_e.ok)return await ha(e,S.fileUrl).catch(()=>{}),Be(_e);try{await ma(e).load(S.fileUrl,{force:!0})}catch(re){console.warn("reload new release file failed:",re?.message||re)}let ye=F(S.fileUrl);e.add(S.releaseNode,ce("type"),Ht,ye),e.add(S.releaseNode,ce("type"),de("Dataset"),ye),e.add(S.releaseNode,O("title"),te(S.title),ye),e.add(S.releaseNode,O("identifier"),te(S.ident),ye),e.add(S.releaseNode,O("isPartOf"),l.releasesCatalog,ye),S.lp&&e.add(S.releaseNode,de("landingPage"),F(S.lp),ye),S.artist&&e.add(S.releaseNode,xe("maker"),N||te(S.artist),ye);for(let re of S.tracks)e.add(re.node,ce("type"),Oi,ye),re.name&&e.add(re.node,O("title"),te(re.name),ye),e.add(re.node,X("item"),F(re.dl),ye),Number.isFinite(re.dur)&&re.dur>0&&e.add(re.node,X("duration"),te(String(re.dur),void 0,bt("decimal")),ye),e.add(re.node,O("isPartOf"),S.releaseNode,ye),e.add(S.releaseNode,X("track"),re.node,ye)}for(let S of pe.values()){let N=await Ae(e,S.fileDoc.value,[],S.inserts);if(!N.ok)return Be(N)}let We=0,P=0;for(let S of e.match(s,K("itemListElement"),null)){let N=parseInt(e.any(S.object,K("position"))?.value,10);Number.isFinite(N)&&(We=Math.max(We,N));let V=S.object.value.match(/#e(\d+)$/);V&&(P=Math.max(P,parseInt(V[1],10)))}let G=[];oe.forEach((S,N)=>{let V=F(`${s.value}#e${p(P+N+1)}`);G.push(M(s,K("itemListElement"),V,s),M(V,ce("type"),K("ListItem"),s),M(V,K("position"),te(String(We+N+1),void 0,bt("integer")),s),M(V,K("item"),S,s))});let ae=160;for(let S=0;S<G.length;S+=ae){let N=await Ae(e,s.value,[],G.slice(S,S+ae));if(!N.ok)return Be(N)}return{ok:!0,nodes:oe,added:Se,skipped:b}}async function ko(e,t,i,r){let n=F(i),l=[],s=null;for(let y of e.match(n,K("itemListElement"),null)){let g=y.object,b=e.any(g,K("item")),h=parseInt(e.any(g,K("position"))?.value,10);if((b&&e.any(b,X("item"))?.value)===r&&!s){s={ent:g,trk:b,pos:h};continue}l.push({ent:g,trk:b,pos:Number.isFinite(h)?h:Number.MAX_SAFE_INTEGER})}if(!s)return{ok:!0};let c=[M(n,K("itemListElement"),s.ent,n),M(s.ent,ce("type"),K("ListItem"),n),M(s.ent,K("position"),te(String(s.pos),void 0,bt("integer")),n),M(s.ent,K("item"),s.trk,n)],p=[];l.sort((y,g)=>y.pos-g.pos).forEach((y,g)=>{let b=g+1;y.pos!==b&&(c.push(M(y.ent,K("position"),te(String(y.pos),void 0,bt("integer")),n)),p.push(M(y.ent,K("position"),te(String(b),void 0,bt("integer")),n)))});let v=await Ae(e,n.value,c,p);if(!v.ok)return v;if(t&&i===Kt(t))try{let y=Te(t),g=n,b=e.any(s.trk,O("isPartOf"))||e.match(null,X("track"),s.trk)[0]?.subject;if(b&&e.holds(b,ce("type"),Ht)){let h=e.match(b,X("track"),null).map(L=>L.object),f=!1;e:for(let L of h)for(let $ of e.match(null,K("item"),L)){let D=e.match(null,K("itemListElement"),$.subject)[0]?.subject;if(D&&D.value!==g.value){f=!0;break e}}if(!f){let L=Ga(e,b,y.releasesDoc),$=[];for(let D of e.match(y.releasesCatalog,de("dataset"),b))$.push(M(D.subject,D.predicate,D.object,y.releasesDoc));for(let D of e.match(y.releasesDoc,ut("seeAlso"),null))D.object.value===L.value&&$.push(M(D.subject,D.predicate,D.object,y.releasesDoc));$.length&&await Ae(e,y.releasesDoc.value,$,[]),await ha(e,L.value).catch(()=>{});for(let D of h){for(let z of e.match(D,null,null))e.remove(z);for(let z of e.match(null,null,D))e.remove(z)}for(let D of e.match(b,null,null))e.remove(D);for(let D of e.match(null,null,b))e.remove(D)}}}catch(y){console.warn("Deleted-bin release GC failed (orphan left for sweep):",y?.message||y)}return{ok:!0}}async function So(e,t,i,r={}){let n=Te(t),l=Ga(e,i,n.releasesDoc),s=[],c=[];if(r.title!=null){for(let p of e.match(i,O("title"),null))s.push(M(p.subject,p.predicate,p.object,l));r.title&&c.push(M(i,O("title"),te(r.title),l))}if(r.artist!=null){for(let p of e.match(i,xe("maker"),null))s.push(M(p.subject,p.predicate,p.object,l));r.artist&&c.push(M(i,xe("maker"),te(r.artist),l))}if(r.album!=null){let p=e.match(null,X("track"),i)[0]?.subject;if(p){let v=Ga(e,p,l);for(let y of e.match(p,O("title"),null))s.push(M(y.subject,y.predicate,y.object,v));r.album&&c.push(M(p,O("title"),te(r.album),v))}}return!s.length&&!c.length?{ok:!0}:Ae(e,l.value,s,c)}function _o(e,t){let i=e.match(null,X("track"),t)[0]?.subject;if(!i)return 0;let r=e.match(i,X("track"),null).length;return Math.max(0,r-1)}function or(e,t){let i=[],r=new Set,n=s=>{e.holds(s,ce("type"),Ht)&&(r.has(s.value)||(r.add(s.value),i.push({name:e.any(s,O("title"))?.value||e.any(s,de("landingPage"))?.value||s.value,url:s.value,_local:!0,_releaseNode:s})))},l=e.any(t,O("source"));if(l){for(let s of e.match(l,K("itemListElement"),null)){let c=e.any(s.object,K("item")),p=c&&e.any(c,O("isPartOf"));p&&n(p)}return i}for(let s of e.match(null,xe("maker"),t))n(s.subject);return i}function Lo(e,t){if(!e||!t)return null;let i=e.match(null,X("item"),F(t));return i.length?i[0].subject:null}function Ao(e,t){let i=[];for(let r of e.match(t,X("track"),null)){let n=r.object,l=e.any(n,X("item"))?.value;if(!l)continue;let s=e.any(n,X("duration"))?.value;i.push({url:l,name:e.any(n,O("title"))?.value||n.value,time:Ol(s),node:n,_lengthSec:s!=null?parseFloat(s):NaN,_bitrate:NaN})}return i}function Ol(e){let t=parseFloat(e);if(!Number.isFinite(t)||t<=0)return"";let i=Math.floor(t/60),r=Math.floor(t%60);return`${i}:${String(r).padStart(2,"0")}`}async function Eo(e,t,i,r={}){let l=Te(t).agentsDoc,s=F(i),c=(r.name||e.any(s,O("title"))?.value||"Untitled Artist").trim(),p=r.genreId;if(!p)return{ok:!1,err:"a genre is required"};let v=new Set;for(let L of e.match(s,K("itemListElement"),null)){let $=e.any(L.object,K("item")),D=$&&e.any($,O("isPartOf"));D&&e.holds(D,ce("type"),Ht)&&v.add(D.value)}let y=e.match(null,O("source"),s)[0]?.subject||mo(e,c),g=y||F(`urn:uuid:${crypto.randomUUID?.()??`${Date.now()}-${Math.random().toString(36).slice(2)}`}`),b=y?e.statementsMatching(y,null,null).filter(L=>(L.why?.value||l.value)===l.value):[],h=[M(g,ce("type"),Wi,l),M(g,xe("name"),te(c),l),M(g,X("genre"),F(p),l),M(g,O("source"),s,l)],f=await Ae(e,l.value,b,h);return f.ok?{ok:!0,node:g,name:c,genreId:p,albumCount:v.size,relinked:!!y}:{...f,node:null}}async function sr(e,t,i){let n=Te(t).agentsDoc,l=F(i),s=e.match(null,O("source"),l)[0]?.subject;if(!s)return{ok:!0,node:null};let c=e.statementsMatching(s,null,null).filter(v=>(v.why?.value||n.value)===n.value),p=await Ae(e,n.value,c,[]);return p.ok?(await lr(e,t,i,!1).catch(()=>{}),{ok:!0,node:s}):{...p,node:s}}async function lr(e,t,i,r){let n=F(i),l=e.statementsMatching(n,Gi("styleClass"),null),s=r?[M(n,Gi("styleClass"),te("hidden"),n)]:[];return!l.length&&!s.length?{ok:!0}:Ae(e,n.value,l,s)}function Hi(e){if(!e)return NaN;let t=String(e).trim();if(!t)return NaN;if(/^[0-9.]+$/.test(t))return parseFloat(t);let i=t.split(":").map(Number);return i.some(r=>!Number.isFinite(r))?NaN:i.length===3?i[0]*3600+i[1]*60+i[2]:i.length===2?i[0]*60+i[1]:i[0]}function To(e){let t=String(e).trim();if(!t)return null;let i=t.match(/archive\.org\/details\/([^\/?\s#]+)/);return i?{id:i[1],url:`https://archive.org/details/${i[1]}`}:/^[a-zA-Z0-9._-]+$/.test(t)?{id:t,url:`https://archive.org/details/${t}`}:null}function $o(e,t){if(typeof customElements>"u")return;let i=customElements.get(e);if(i){i!==t&&!window.__SolSuppressDefineWarn&&console.warn(`[sol-components] <${e}> already registered; keeping the existing definition.`);return}customElements.define(e,t)}var Qa=class extends HTMLElement{_on(t,i,r,n){return t.addEventListener(i,r,n),this.__cleanups||(this.__cleanups=[]),this.__cleanups.push(()=>t.removeEventListener(i,r,n)),r}_cleanup(t){return typeof t!="function"||(this.__cleanups||(this.__cleanups=[]),this.__cleanups.push(t)),t}disconnectedCallback(){let t=this.__cleanups;if(this.__cleanups=null,t)for(let i of t)try{i()}catch{}}};var Hl='a[href], button:not([disabled]), input:not([disabled]), select:not([disabled]), textarea:not([disabled]), [tabindex]:not([tabindex="-1"])';function cr(e){return e?Array.from(e.querySelectorAll(Hl)):[]}function dr(){let e=document.activeElement;for(;e&&e.shadowRoot&&e.shadowRoot.activeElement;)e=e.shadowRoot.activeElement;return e}function Co(e,t,i){if(e.key!=="Tab"||!t)return!1;let r=i?i():cr(t);if(!r.length)return!1;let n=r[0],l=r[r.length-1],s=dr();return e.shiftKey&&s===n?(l.focus(),e.preventDefault(),!0):!e.shiftKey&&s===l||!t.contains(s)?(n.focus(),e.preventDefault(),!0):!1}var Gl=`
